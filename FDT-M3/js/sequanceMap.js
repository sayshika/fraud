...
function sequanceMapFunc(featureCollection){
	...
	L.tileLayer(MB_URL, {
 	...
 	var grayscale = L.tileLayer(MB_URL, {attribution: MB_ATTR, id: 'examples.map-20v6612k'});
	var streets = L.tileLayer(MB_URL, {attribution: MB_ATTR, id: 'examples.map-i876mjb7'});

	var timeArr = [];
	function onEachFeature(feature, layer) {
		//console.log("inside onEachFeature1");
		var popupContent = "";
		timeArr.push(feature.properties.payload_timestamp);
		...
		$.each(timeArr,function(index,e){
			...
			if(e == feature.properties.payload_timestamp){
				layer.bindLabel(" " + seq + " ", { noHide: true,direction: 'right'});
				//console.log(e+" - "+seq);
			}
			});
